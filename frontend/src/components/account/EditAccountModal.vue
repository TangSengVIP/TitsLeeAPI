  ...
  >
    ...
    >
      ...
      <div v-if="account.type === 'upstream'" class="space-y-4">
        ...
        <!-- Fetch Models button (only for upstream type with account ID) -->
        <div>
          <button
            type="button"
            :disabled="!account?.id || !editBaseUrl?.trim() || (!editApiKey?.trim() && !props.account?.credentials?.api_key) || fetchingUpstreamModels"
            @click="fetchUpstreamModelsForEdit"
            :class="[
              'flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors',
              (editBaseUrl?.trim() && (editApiKey?.trim() || props.account?.credentials?.api_key))
...
const fetchUpstreamModelsForEdit = async () => {
  if (!props.account?.id) return
  fetchingUpstreamModels.value = true
  upstreamModelsFetchError.value = ''
  upstreamModelsResult.value = null
  try {
    const result = await adminAPI.accounts.fetchUpstreamModels(props.account.id)
    upstreamModelsResult.value = result
    ...
    appStore.showInfo(t('admin.accounts.upstreamModelsAutoFilled', { count: result.count }))
  } catch (err: any) {
    upstreamModelsFetchError.value = err?.response?.data?.error || err.message || t('admin.accounts.fetchUpstreamModelsFailed')
  } finally {
